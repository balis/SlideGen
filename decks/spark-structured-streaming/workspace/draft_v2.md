...
- Structured Streaming **reuses Spark SQL's Catalyst optimizer and Tungsten code generation**, so most optimizations — such as predicate pushdown, projection pushdown, and expression simplification — apply automatically [claim:sss:catalyst_reuse:005]
...
- Exactly-once is guaranteed by the combination of **two persistent logs**, **replayable sources**, and **idempotent sinks** [claim:sss:wal_exactly_once:010]:
...
| **Checkpointing** | Full snapshot each version | Full snapshot (default) or changelog (opt-in, Spark 3.3+) |
...
| **Latency** | ~100 ms floor [claim:sss:microbatch_latency:013] | ~1 ms [claim:sss:continuous_mode_latency:014] | Very low (per-event) |
...
| **State access** | State store (remote) | N/A (stateless ops only) | Local, often in-memory state [claim:sss:flink_per_event:018] |
